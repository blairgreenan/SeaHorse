...
XX <- read.oce(file="DPL5_000.000")
YY <- applyMagneticDeclination(object = XX, declination = 117.57, debug = 1)
DPL5_east <- YY@data$v[,,1]
DPL5_tibble_east <- as_tibble(DPL5_east)
DPL5_north <- YY@data$v[,,2]
DPL5_tibble_north <- as_tibble(DPL5_north)
# Compute the depth-averaged mean for each profile
rmean <- rowMeans(DPL5_tibble_east, na.rm = TRUE)
dev.new()
plot(rmean, type="l", lty=1)


XXX <- read.oce(file="DPL6_000.000")
YYY <- applyMagneticDeclination(object = XXX, declination = 117.57, debug = 1)
DPL6_east <- YYY@data$v[,,1]
DPL6_tibble_east <- as_tibble(DPL6_east)
DPL6_north <- YYY@data$v[,,2]
DPL6_tibble_north <- as_tibble(DPL6_north)
# Compute the depth-averaged mean for each profile
rmean <- rowMeans(DPL6_tibble_east, na.rm = TRUE)
dev.new()
plot(rmean, type="l", lty=1)
...
if (From_Scratch){
  # load ADCP Data from the .mat file
  SeaHorse_ADCP <- readMat("DPL5_000.mat")
